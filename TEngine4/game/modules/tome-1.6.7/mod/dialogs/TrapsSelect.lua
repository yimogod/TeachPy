...
game.log("starting trap selection dialog")
...
function _M:use(item)
	...
	if item.tid and item.tier then -- toggle trap selection
		...
		if item.unlearnable and not self.traps_selected[tid] then
			game.logPlayer(self.actor, "#LIGHT_BLUE#You cannot prepare this trap: %s.", item.unlearnable)
		elseif item.tier > self.mastery_level and not self.traps_selected[tid] then
			game.logPlayer(self.actor, "#LIGHT_BLUE#You need more skill to prepare this trap.")
		else
			if tid == self.actor.trap_primed then
				game.logPlayer(self.actor, "#LIGHT_BLUE#Preparing trap with normal trigger.")
			end
	...
	local list = {{label="Accept these selections", tier=0, color = colors_simple.GOLD,
		action=function()
			if self.num_sel <= self.max_traps then
				...
				game.logPlayer(self.actor, "#LIGHT_BLUE#You cannot prepare more than %d traps.", self.max_traps)
			end
		end}}
